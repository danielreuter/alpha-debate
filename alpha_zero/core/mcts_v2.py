...
def uct_search(
    ...
    Args:
        env: a gym like custom PixelDebateEnv environment.
    ...
    Raises:
        ValueError:
            if input argument `env` is not valid PixelDebateEnv instance.
            if input argument `num_simulations` is not a positive integer.
        RuntimeError:
            if the game is over.
    """
    if not isinstance(env, PixelDebateEnv):
        raise ValueError(f'Expect `env` to be a valid PixelDebateEnv instance, got {env}')
