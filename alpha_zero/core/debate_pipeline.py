...
def run_evaluator_loop(
    ...
    liar_player = create_mcts_player(
        network=network,
        device=device,
        num_simulations=num_simulations,
        num_parallel=num_parallel,
    ...
    honest_player = create_mcts_player(
...
def eval_against_prev_ckpt(
    ...
    while not done:
        if env.to_play == env.liar_player:
            mcts_player = liar_player
        else:
            mcts_player = honest_player
        move, *_ = mcts_player(
            env=env,
            root_node=None,
            c_puct_base=c_puct_base,
    ...
    if env.winner is not None:
        if env.winner == env.liar_player:
            winner, loser = black_elo, white_elo
        elif env.winner == env.honest_player:
