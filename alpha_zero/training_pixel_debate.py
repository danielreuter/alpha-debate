...
# Pixel Debate Flags
flags.DEFINE_integer('num_pixels', 6, 'Number of pixels to reveal in the debate game.')
flags.DEFINE_bool('precommit', True, 'Whether to use precommitment in the debate game.')

# Judge Model Flags
flags.DEFINE_string('judge_dir', './checkpoints/debate/mnist/judge', 'Path for checkpoint file.')
flags.DEFINE_string('judge_name', '{num_pixels}_pixel_judge.pth', 'Name format for the judge model file.')
...
flags.DEFINE_string('ckpt_dir', './checkpoints/debate/mnist', 'Path for checkpoint file.')
flags.DEFINE_string(
    'logs_dir',
    './logs/debate/mnist',
...
flags.DEFINE_string(
    'eval_games_dir',
    './games/pro_games/debate/mnist',
    'Path contains evaluation games in sgf format.',
)
flags.DEFINE_string(
    'save_sgf_dir',
    './games/selfplay_games/debate/mnist',
...
from alpha_zero.core.simple_judge import get_judge
from alpha_zero.envs.mnist import PixelDebateEnv
from alpha_zero.core.debate_pipeline import (
    run_learner_loop,
    run_evaluator_loop,
...
def main():
    ...
    def env_builder():
        judge_path = os.path.join(os.path.abspath(FLAGS.judge_dir), FLAGS.judge_name.format(num_pixels=FLAGS.num_pixels))
        judge = get_judge(FLAGS.num_pixels, judge_path)
        return PixelDebateEnv(max_turns=FLAGS.num_pixels, precommit=FLAGS.precommit, judge=judge)
